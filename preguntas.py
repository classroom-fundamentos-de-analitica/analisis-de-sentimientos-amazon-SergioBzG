...
def pregunta_01():
    ...
    df_tagged = df[df["tag"].notnull()]
    df_untagged = df[df["tag"].isnull()]

    x_tagged = df_tagged["text"]
    y_tagged = df_tagged["tag"]

    x_untagged = df_untagged["text"]
    y_untagged = df_untagged["tag"]
